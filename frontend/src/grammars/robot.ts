export const grammar = {
  ...
  repository: {
    ...
    settings_table_setting_imports: {
      begin:
        '(?i)^(Library|Resource|Variables|Metadata)(?: {2,}| ?\\t)+(.*?)(?= {2}| ?\\t| ?$)',
      beginCaptures: {
        '1': {
          name: 'storage.type.setting.robot',
        },
    ...
    settings_table_setting_keywordCalls: {
      begin:
        '(?i)^(Suite Setup|Suite Teardown|Test Setup|Task Setup|Test Teardown|Task Teardown|Test Template|Task Template)(?: {2,}| ?\\t)+(.*?)(?= {2}| ?\\t| ?$)',
      beginCaptures: {
        '1': {
          name: 'storage.type.setting.robot',
        },
    ...
    test_cases_table: {
      begin:
        '(?i)^(\\*+ ?(?:Test Cases|Test Case|Tasks|Task)[ *]*)(?= {2,}| ?\\t| ?$)',
      beginCaptures: {
        '1': {
          name: 'entity.name.type.class.robot',
        },
    ...
    run_keyword_if: {
      ...
      begin:
        '(?i)^(?:\\s)+((?:Run Keyword If)|(?:Run Keyword Unless))(?:(?: {2,})|(?: ?\\t))+(.*?)?(?:  | ?\\t| ?$)+(.*?)?(?:  | ?\\t| ?$)+',
      beginCaptures: {
        '1': {
